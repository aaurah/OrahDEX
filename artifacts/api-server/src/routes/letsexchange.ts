...
// On a cold cache it waits (up to 12 s) for the key-coin fetch so the response
// always carries real prices.  Awaiting matters inside Cloudflare Workers:
// fire-and-forget promises are cancelled when the response finishes, so the
// old "return {} and warm in the background" pattern never populated the cache.
router.get("/letsexchange/usd-prices", async (_req, res) => {
  let prices = getCachedLEPrices();
  if (Object.keys(prices).length === 0) {
    try {
      prices = await Promise.race([
        fetchLEKeyPricesIfNeeded(),
        new Promise<Record<string, number>>((_, rej) =>
          setTimeout(() => rej(new Error("timeout")), 12000)
        ),
      ]);
    } catch {
      prices = getCachedLEPrices();
    }
  }
  res.json(prices);
});
