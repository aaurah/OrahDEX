...
// ── Safety middleware: hard 60-second deadline on every AI route ──────────────
...
// NOTE: this router is mounted on the shared /api router, so the deadline
// applies to ALL API routes. 25 s was too short for legitimately heavy
// endpoints like /api/letsexchange/pairs?all=true (~100k pairs, ~20–40 s on a
// cold isolate) — the watchdog fired first and returned a bogus "AI service
// timeout" 503. 60 s covers those while still bounding true hangs.
