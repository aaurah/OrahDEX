...
import { useOnlineStatus, useRetryCountdown, retryNow } from "@/hooks/useOnlineStatus";

/**
 * Thin status strip fixed to the top of the screen.
 *
 * - Offline  → amber bar: "Connection lost — retrying in Xs" + "Retry now" button
...
export function ApiConnectionBanner() {
  ...
  const retryIn = useRetryCountdown();
    ...
    >
      {phase === "offline" ? (
        <>
          ...
          <span>
            Connection lost —{" "}
            {retryIn > 1 ? `retrying in ${retryIn}s` : "retrying…"}
          </span>
          <button
            onClick={retryNow}
            className="ml-1 px-2 py-0.5 rounded bg-amber-950/20 hover:bg-amber-950/40 active:scale-95 transition-all font-semibold"
          >
            Retry now
          </button>
        </>
