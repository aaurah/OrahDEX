...
function NotificationBell({ counts }: { counts: NotifCounts | undefined }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className="relative p-2 rounded-xl border border-border text-muted-foreground hover:text-foreground hover:border-primary/30 transition-all"
        title="Notifications"
      >
        <Bell className="w-4 h-4" />
        {total > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[16px] h-4 flex items-center justify-center px-1 rounded-full bg-red-500 text-[9px] font-black text-white shadow-lg shadow-red-500/30 tabular-nums leading-none">
            {total > 99 ? "99+" : total}
          </span>
        )}
      </button>
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 w-64 bg-card border border-border rounded-2xl shadow-2xl shadow-black/40 z-50 overflow-hidden">
            <div className="px-4 py-3 border-b border-border flex items-center justify-between">
              ...
              {total > 0 && (
                <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-red-500/10 border border-red-500/20 text-red-400">
                  {total} pending
                </span>
              )}
            </div>
            ...
            {activeItems.length === 0 ? (
              <div className="px-4 py-6 text-center">
                ...
                <p className="text-[10px] text-muted-foreground">No pending notifications</p>
              </div>
            ) : (
              <div className="py-1">
                {items.map(item => (
                  <button
                    ...
                    className={cn(
                      ...
                      item.count > 0 ? "hover:bg-white/[0.05]" : "opacity-40 pointer-events-none",
                    )}
                  >
                    <div className={cn(
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-muted-foreground leading-tight">
                        {item.count === 0 ? "None" : `${item.count} item${item.count !== 1 ? "s" : ""}`}
                      </p>
                    </div>
                    {item.count > 0 && (
                      <span className="text-[9px] font-black min-w-[20px] text-center px-1.5 py-0.5 rounded-full bg-red-500 text-white shadow-lg shadow-red-500/30 tabular-nums leading-none">
...
export function AdminLayout({ children }: { children: ReactNode }) {
  ...
  const { data: notifCounts } = useQuery<NotifCounts>({
    queryKey: ["admin-notif-counts"],
    queryFn:  () => adminFetch("/api/admin/notifications/counts").then(r => r.json()),
                ...
                {!isCollapsed && (
                  <div className="space-y-0.5">
                    {group.items.map(item => {
                      ...
                      const navBadgeCount =
                        item.href === "/admin/support/inbox" ? (notifCounts?.unreadTickets ?? adminUnreadCount) :
                        item.href === "/admin/mail"          ? (notifCounts?.unreadEmails ?? 0) :
                        item.href === "/admin/withdrawals"   ? (notifCounts?.pendingWithdrawals ?? 0) : 0;
                      return (
                        <Link
                          key={item.href}
                          href={item.href}
