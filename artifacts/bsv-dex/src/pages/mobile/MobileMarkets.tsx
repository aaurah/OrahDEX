...
function getCatRows(
  ...
  switch (cat) {
    case "all": {
      // DB-first merge: every live /api/markets pair, then mock fills for
      // coins not yet in the DB, then any already-loaded LE pairs whose base
      // coin isn't covered yet. Previously this returned ONLY the static mock
      // pool, so the All tab showed a few hundred stale rows instead of the
      // full live market list.
      const merged = new Map<string, MktRow>();
      for (const m of apiRows) {
        if (m.type !== "futures" && m.price > 0) merged.set(m.symbol, m);
      }
      for (const m of enrich(MOBILE_ALL_POOL)) {
        if (m.type !== "futures" && m.price > 0 && !merged.has(m.symbol))
          merged.set(m.symbol, m);
      }
      if (leAllPairs.length > 0) {
        const coveredBases = new Set(Array.from(merged.values()).map(r => r.base));
        const leExtraByBase = new Map<string, MktRow>();
        for (const p of leAllPairs) {
          if (p.price <= 0 || coveredBases.has(p.base)) continue;
          const cur = leExtraByBase.get(p.base);
          if (!cur || p.price > cur.price) leExtraByBase.set(p.base, p);
        }
        for (const row of leExtraByBase.values()) {
...
export function MobileMarkets() {
  ...
  // LE "all" pairs — needed for All/ETH/SOL tabs, Favorites, or when searching.
  // Served from the worker's KV cache (gzipped ~1 MB), so it's cheap on mobile.
  const needAllPairs = cat === "all" || cat === "eth" || cat === "sol" || cat === "favorites" || search.length > 0;
