...
- **Stable Markets List (Option A+B)**: `markets` table is now the single stable source of truth for all pair listings. Two new columns: `enabled BOOLEAN` (soft-hide without deleting) and `pinned BOOLEAN` (true = internal spot/futures pair, never auto-removed). All 4,003 internal (spot/futures) pairs are pinned=true. All 32,036 LetsExchange pairs are pinned=false, enabled=true. `GET /markets` now always filters by `enabled=true` and supports `?category=internal` (spot+futures), `?category=external` (letsexchange), `?type=spot,futures` (comma-separated). New admin endpoint: `PATCH /admin/markets/:symbol/enabled` (soft enable/disable). `seedMarketsIfNeeded()` runs `UPDATE markets SET pinned=true WHERE type IN ('spot','futures') AND pinned=false` after each seed cycle — idempotent, self-healing. `syncAllLEPairs()` upsert protected: only updates price columns for `letsexchange` rows, never touches `pinned`/`enabled`. Pairs no longer appear/disappear on refresh.
- **Hybrid Swap Router v3** (`artifacts/api-server/src/lib/hybridRouter.ts`): ROUTE_VERSION="v3". DB-backed per-pair config from `routing_profiles` table (60 s TTL in-memory cache; `invalidatePairConfigCache()` exported for admin use; tier defaults used on DB miss). Routing algorithm: (1) Check `enabled` flag and `maxInternalSize` cap, (2) Check oracle status — stale (>5 min) or missing counts as unavailable, (3) Walk real orderbook (`is_bot=false`, `is_synthetic=false`) → VWAP fill simulation, (4) Route internal if fill ≥ `minFillFraction` AND slippage ≤ `maxSlippage`, (5) Route split if `allowSplit=true` AND `split_enabled=true` AND fill > 0 AND ≥ `MIN_SPLIT_INTERNAL_FRACTION` (5%), (6) Route external otherwise. Oracle status values: `"available" | "missing" | "stale"` — all routing logs include `oracleStatus`. Split routing: `source="split"` with `splitLegs: { internal: SplitLeg, external: SplitLeg }` — internal leg uses VWAP book walk, external leg sends remainder to LetsExchange. DB schema: `routing_profiles` table (10 rows seeded: BSV/USDT + BSV/BTC split enabled; stable pairs oracle_required=false). Orders classified by `is_bot`/`is_synthetic` columns (backfilled, 0 mismatches verified). `POST /swap/route` accepts `allowSplit` param and returns full `pairConfig` + `splitLegs`. `POST /swap/execute`: handles all three sources (internal/letsexchange/split), `forceSource` guard (X-Internal-Token header, silently ignored + WARN log for untrusted callers), `allowSplit` forwarded to router. Admin CRUD: `GET/POST/PUT /admin/routing-profiles` (requires admin token, invalidates cache on write). Frontend badge in `LetsExchangePanel`: v2 UI (split badge + 11px muted subtext, explicit `routeError` state, 600ms debounce).

## Feature Specifications

The platform supports 958 markets (spot + perpetuals across 10 EVM chains + BSV/BTC/SOL/TRON) with 210 live price symbols. Trading features include various order types (Limit, Market, Stop-Limit, etc.), real-time order books, TradingView charts, and dynamic fee displays. Futures trading offers leverage, cross/isolated margin, live mark/index prices, and funding rates. EVM chain support for 10 chains is integrated throughout. Wallet Connect supports multiple BSV wallets and Reown AppKit for EVM. Balance guards prevent overdrafts. A push notification system is implemented. The Admin AI Intelligence panel offers model selection, prompt preview, insights, trade signals, and a chat tester. Ticker API provides comprehensive market data including `markPrice`, `indexPrice`, and `openInterest`. BSV price is live from a sovereign engine. OrahNFT is a social NFT marketplace (Instagram×Zora-style) where every post is a BSV inscription and a tradeable creator coin on bonding curves. Social DB tables (`social_posts`, `creator_profiles`, `creator_coins`, `coin_holdings`, `coin_trades`, `post_comments`, `post_mints`, `post_likes`, `social_follows`) are live in PostgreSQL. NFT tab includes a real-time Community Chat panel using the existing SSE chat infrastructure (channel `"nft"`) via `/api/chat/channels/nft/*`. Fiat on-ramp supports 6 providers (MoonPay, Transak, Banxa, Simplex, Ramp, Mercuryo) with Apple Pay, Google Pay, Card, and Bank Transfer.
