...
- **Orah Native HD Wallet**: Supports BIP39 and BIP44/SLIP-0010 for multiple chains (EVM, BTC/BSV/BCH, SOL) using `@scure/bip32`, `@scure/bip39`, `@noble/curves`, `@noble/hashes`. BTC, BSV, and BCH all derive from the same BIP44 path (`m/44'/0'/0'/0/0`) producing identical legacy P2PKH addresses ("1...") across all three Bitcoin forks. For Phantom wallet users, the hook detects Phantom's Bitcoin provider (`window.phantom.bitcoin`) and uses the user's real BTC address from Phantom for all three forks — ensuring the address displayed in Orah matches what users see in their Phantom wallet. For other external EVM wallets (MetaMask, etc.), a server-generated custodial sub-account is created via `/api/user/bsv-wallet`. Allows import via BIP39 seed phrase or EVM private key.
- **Non-Custodial Balance Architecture**: External wallets (EVM/MetaMask, BSV, SOL) use on-chain RPC balance reads exclusively (`useEvmBalances` via `eth_getBalance`/`balanceOf`, `useBsvBalance` via WhatsOnChain polling every 30s). `useWalletStore.balance` is persisted only for non-EVM chains; EVM wallets always fall back to 0 before the RPC hook resolves. `useExchangeBalanceStore` (internal ledger + `applyFill`) is used solely for the OrahDEX internal wallet path (`isOrahDEXWallet = provider === "orahdex-wallet"`). Fills settle directly to the user's external wallet address — no internal crediting. `DepositModal.tsx` and `WithdrawModal.tsx` component files have been deleted; `ReceiveModal.tsx` replaced them with a pure wallet-address QR viewer.
- **Unified API Ledger Trading** (OrahDEX wallet only): The OrahDEX internal wallet uses the API ledger for balance tracking and order funding. Limit orders lock funds via `lockForOrder` (available → locked in `user_balances`). Balance auto-seeds on first trade. After each trade, balances are refreshed from the API.
...
- **AMM Liquidity System**: Utilizes a standard `Δy = (Δx × (1−fee) × y) / (x + Δx × (1−fee))` formula with a defined `LP_FEE_RATIO` and `PROTOCOL_FEE_RATIO`. LP positions are managed via `useLiquidityStore` (Zustand). Orah native AMM contracts (`OrahDEXPair.sol` ERC-20 LP token, `OrahDEXFactory.sol` CREATE2 pair registry, `OrahDEXRouter02.sol` liquidity router) are compiled via Hardhat in `artifacts/orah-contracts/` and **deployed on Sepolia testnet (2026-04-16)**:
  ...
  Frontend hooks: `useLpBalance.ts` reads on-chain LP token balance via raw `eth_call` (no deps). `orahdexAmmAddresses.ts` is the central registry of deployed addresses + inline ABI fragments. `OnChainLpBadge` in `Liquidity.tsx` shows real-time on-chain LP balance in the "My Positions" tab. `PositionEntry` extended with `lpTokenAddress` field. To add a new chain: run `pnpm --filter @workspace/orah-contracts deploy:<network>` and paste addresses into `orahdexAmmAddresses.ts`.
...
- **EVM HTLC Atomic Settlement**: Utilizes the `OrahHTLC.sol` Solidity contract for atomic swaps on Ethereum, Polygon, and BSC, supporting native ETH and ERC-20 tokens. A `evm_htlc_sessions` DB table tracks swap details, and `evmHtlc.ts` service manages session initiation, lock confirmation, and on-chain watching. API endpoints and frontend hooks (`useEvmHtlcSession`, `HTLCSettlementCard`) facilitate the user experience. The design ensures non-custodial operation for external wallets with asymmetric timelocks for security.
...
- Internal wallets (orahdex-wallet, demo, passkey, mobile-qr) don't have wagmi connectors and cannot sign via `@wagmi/core`
...
- **Bug**: `useEvmBalances` hook only fetches on-chain EVM balances; internal wallets (demo/BSV/orahdex-wallet/passkey/mobile-qr) returned empty balances causing "Insufficient balance" on every deposit
...
- Balance source selection uses `walletProvider` (not just `network`/address prefix): `INTERNAL_PROVIDERS = ["demo", "orahdex-wallet", "passkey", "mobile-qr"]` → use backend balances; external EVM wallets → use on-chain EVM balances
...
Three issues combined to prevent the app loading on mobile Safari (orah.org):
