...
- **Hybrid Swap Router v2** (`artifacts/api-server/src/lib/hybridRouter.ts`): Production-grade VWAP + slippage routing engine (ROUTE_VERSION="v2"). Routing algorithm: (1) walk real orderbook (is_bot=false, is_synthetic=false) to simulate fill, (2) compute VWAP, (3) compare to oracle → implied slippage, (4) route internal if fill ≥ minFillFraction AND slippage ≤ maxSlippage, else route to LetsExchange. Per-pair config (tiered: MAJOR/ALT/STABLE + explicit BSV overrides); future migration to DB table planned. Oracle failure policy: explicit fallback per pair ("letsexchange" conservative default, "internal", or "internal_below_size"). Partial fill contract: `fillBehavior="reject_partial"` — whole order routes to LE if threshold not met; no split routing yet. Orders classified by `is_bot`/`is_synthetic` DB boolean columns (backfilled, 0 mismatches verified). All routing decision logs include `routeVersion`, no PII (wallet addresses masked as `walletMask`). Endpoints: `POST /api/swap/route` (returns VWAP, slippage, fillBehavior, pairConfig, oracleFallbackApplied, routeVersion, fee breakdown + assumption docs, effectiveRate), `POST /api/swap/execute` (forceSource guard: X-Internal-Token header required; silently ignored for public callers with WARN log). Frontend badge in `LetsExchangePanel`: split into main badge line + explanatory subtext; explicit "Routing check unavailable" error state; `routeError` resets on every new debounced request.
