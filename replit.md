...
- **OrahDEX Native HD Wallet**: Supports BIP39 and BIP44/SLIP-0010 for multiple chains (EVM, BTC/BSV/BCH, SOL) using `@scure/bip32`, `@scure/bip39`, `@noble/curves`, `@noble/hashes`. BTC, BSV, and BCH all derive from the same BIP44 path (`m/44'/0'/0'/0/0`) producing identical legacy P2PKH addresses ("1...") across all three Bitcoin forks. External EVM wallets (MetaMask, Phantom) also get the same shared legacy address via `/api/user/bsv-wallet`. Allows import via BIP39 seed phrase or EVM private key.
