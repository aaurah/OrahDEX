...
- **Hybrid Swap Router** (`artifacts/api-server/src/lib/hybridRouter.ts`): VWAP-based routing engine that decides between OrahDEX internal orderbook and LetsExchange cross-chain bridge. Routing is based on: (1) walking real (non-bot, non-synthetic) orders to simulate the fill, (2) computing volume-weighted average price (VWAP), (3) comparing VWAP to oracle mid-price to derive slippage, (4) routing internal if fill ≥ 90% AND slippage ≤ 1.5%, otherwise routing via LetsExchange. Orders have explicit `is_bot` and `is_synthetic` boolean columns (not string-matched). Every routing decision is structured-logged with `event: "hybrid_route"`. Endpoints: `POST /api/swap/route` (read-only routing check, returns VWAP, slippage, fee breakdown, effective rates), `POST /api/swap/execute` (unified dispatcher; `forceSource` override requires `X-Internal-Token` header matching `INTERNAL_API_TOKEN` env var). Frontend badge in `LetsExchangePanel` is explicitly informational ("Internal liquidity available — trading via LetsExchange for cross-chain execution") with error state.
