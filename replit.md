...
- **Unified API Ledger Trading**: All wallet types (Phantom, MetaMask, Orah HD, demo) use the internal API ledger for balance tracking and order funding. No wallet signing permission is required for trades. Limit orders lock funds via `lockForOrder` (available → locked in `user_balances`), and the available balance displayed in the UI reflects locked amounts. Balance auto-seeds on first trade. The "In open orders" row shows locked funds. After each trade, balances are refreshed from the API.
...
- **Demo Account**: Activated via `/api/demo/activate` to seed virtual funds. API rejects non-demo addresses for real trading. Demo mode is gated behind wallet connection — accessible only via WalletConnectModal "Demo Account" tab.
