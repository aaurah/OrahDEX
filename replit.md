...
- **Admin Panel**: Features an updated Dashboard with Quick Actions, dynamic system alerts, an "API Settings" page, and improved integrations management. Feature flags are now database-persisted via `/api/admin/site-settings`. Admin list (`/api/admin/admins`) and API keys (`/api/admin/api-keys`) are now DB-backed (stored in `platform_settings`) so data persists across server restarts. **API key hardening (May 2026)**: introduced `middleware/apiKeyAuth.ts` which (a) validates `X-API-Key` / `Authorization: Bearer …` headers against stored hashes on every `/api/*` and `/v1/*` request, attaching `req.apiKey` for downstream rate-limit/scope use, (b) buffers per-key call counts in memory and flushes to DB every 30 s with rolling 24 h reset, populating `calls24h` and `lastUsedAt`. Generation now uses `crypto.randomBytes(24).toString("base64url")` for ~192-bit entropy; the server stores only `sha256(key)` + a `keyPreview` and returns the cleartext exactly once with `oneTimeReveal:true`. Read-modify-write of the JSON-in-DB store is wrapped in a Postgres advisory lock (`pg_advisory_lock`) to eliminate the lost-update race. Server-side input validation rejects empty/oversize names and clamps `rateLimit` to 10–10 000. The four-row mock seed was removed in favour of an empty list. Frontend `ApiSettings.tsx` shows a dedicated one-time reveal modal after generation and lists keys with `keyPreview` + a "hash-only · cannot be re-displayed" note (legacy plaintext keys still authenticate via fallback in `findMatch`). Double-prefix route path bugs fixed for `/api/admin/le-income` and `/api/admin/routing-profiles`. All `console.log/warn/error` calls replaced with structured pino logger throughout `adminAuth.ts` and `admin.ts`.
